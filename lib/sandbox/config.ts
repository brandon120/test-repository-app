...
export function validateEnvironmentVariables(
  ...
  if (selectedAgent === "opencode") {
    ...
    if (!hasAiGateway && !hasAnthropic && !hasOpenAi) {
      errors.push(
        "OpenCode requires AI_GATEWAY_API_KEY, ANTHROPIC_API_KEY, or OPENAI_API_KEY.",
      );
    }
  }
  ...
  if (!credentials.token && !hasOidc) {
    errors.push(
      "Sandbox credentials missing. Set SANDBOX_VERCEL_* vars or deploy on Vercel for OIDC.",
    );
  }
