...
import { appendMessage, getTask, updateTask, } from "./store.js";
...
function createCommitMessage(prompt) {
...
async function runAgentCycle(options) {
    ...
    const agentResult = await executeAgentInSandbox(sandbox, sanitizePrompt(prompt), task.selectedAgent, logger, task.selectedModel, undefined, async () => isTaskStopped(task.id), undefined, isResumed, sessionId, task.id, agentMessageId);
    ...
    const pushResult = await pushChangesToBranch(sandbox, task.branchName, createCommitMessage(prompt), logger);
    if (pushResult.pushFailed) {
        await logger.updateStatus("error", "Changes were committed locally but could not be pushed. Check GITHUB_TOKEN permissions.");
...
export async function continueTask(taskId, message) {
    const task = await getTask(taskId);
    if (!task) {
        throw new Error(`Task ${taskId} not found`);
    }
    ...
    try {
        ...
        if (!sandbox) {
            ...
            const sandboxResult = await createSandbox({
                ...
                enableBrowser: task.enableBrowser,
                preDeterminedBranchName: task.branchName,
                onProgress: async (progress, statusMessage) => logger.updateProgress(progress, statusMessage),
                onCancellationCheck: async () => isTaskStopped(taskId),
            }, logger);
            if (!sandboxResult.success || !sandboxResult.sandbox) {
                throw new Error(sandboxResult.error ?? "Failed to create sandbox");
            ...
            await updateTask(taskId, {
                sandboxName: sandboxResult.sandbox.name,
                sandboxUrl: sandboxResult.domain,
            });
        }
        if (!sandbox) {
            throw new Error("Failed to obtain sandbox");
        }
        await runAgentCycle({
            task,
            prompt: message,
