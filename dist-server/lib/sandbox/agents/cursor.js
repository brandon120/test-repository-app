import { runCommandInSandbox, runInProject, PROJECT_DIR } from '../commands.js';
import { redactSensitiveInfo } from '../../utils/logging.js';
import { generateId } from '../../utils/id.js';
import { initAgentMessage, setAgentMessage } from '../../tasks/agent-messages.js';
// Helper function to run command in sandbox root (for installation checks)
async function runAndLogCommandRoot(sandbox, command, args, logger) {
...
export async function executeCursorInSandbox(sandbox, instruction, logger, selectedModel, mcpServers, isResumed, sessionId, taskId) {
    try {
        // Executing Cursor CLI with instruction
        // Check if Cursor CLI is already installed (for resumed sandboxes)
        const existingCliCheck = await runCommandInSandbox(sandbox, 'sh', ['-c', 'export PATH="$HOME/.local/bin:$PATH"; which cursor-agent 2>/dev/null']);
        let cursorInstall = { success: true };
        ...
        const cliCheck = await runAndLogCommandRoot(sandbox, 'sh', ['-c', 'export PATH="$HOME/.local/bin:$PATH"; which cursor-agent'], logger);
        if (!cliCheck.success) {
...
EOF`;
        ...
        const preExecCheck = await runAndLogCommandRoot(sandbox, 'sh', ['-c', 'export PATH="$HOME/.local/bin:$PATH"; which cursor-agent'], logger);
        if (logger) {
        ...
        const agentMessageId = taskId ? generateId(12) : null;
        if (taskId && agentMessageId) {
            await initAgentMessage(taskId, agentMessageId);
        }
                                ...
                                if (parsed.type === 'tool_call') {
                                    ...
                                    if (parsed.subtype === 'started') {
                                        ...
                                        if (statusMsg) {
                                            ...
                                            void setAgentMessage(taskId, agentMessageId, accumulatedContent);
                                        }
                                    }
                                }
