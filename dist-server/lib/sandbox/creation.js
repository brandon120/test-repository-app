...
import { validateEnvironmentVariables, createAuthenticatedRepoUrl } from './config.js';
...
import { registerSandbox } from './sandbox-registry.js';
import { getSandboxCredentials } from './credentials.js';
// Helper function to run command and log it
async function runAndLogCommand(sandbox, command, args, logger, cwd) {
...
export async function createSandbox(config, logger) {
    try {
...
SKILL_EOF`;
        ...
        if (config.preDeterminedBranchName) {
            ...
            if (branchExistsLocal.success) {
                ...
                const checkoutBranch = await runAndLogCommand(sandbox, 'git', ['checkout', config.preDeterminedBranchName], logger, PROJECT_DIR);
                ...
                if (branchExistsRemote.success && branchExistsRemote.output?.trim()) {
                    ...
                    if (!fetchBranch.success) {
                        ...
                        const checkoutTracking = await runAndLogCommand(sandbox, 'git', ['checkout', '-b', config.preDeterminedBranchName, '--track', `origin/${config.preDeterminedBranchName}`], logger, PROJECT_DIR);
                        ...
                        const checkoutRemoteBranch = await runAndLogCommand(sandbox, 'git', ['checkout', config.preDeterminedBranchName], logger, PROJECT_DIR);
                    ...
                    const createBranch = await runAndLogCommand(sandbox, 'git', ['checkout', '-b', config.preDeterminedBranchName], logger, PROJECT_DIR);
