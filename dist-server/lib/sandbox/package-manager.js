import { runInProject } from './commands.js';
// Helper function to detect package manager based on lock files
export async function detectPackageManager(sandbox, logger) {
...
export async function installDependencies(sandbox, packageManager, logger) {
    let installCommand;
    let logMessage;
    ...
    if (installResult.success) {
        ...
        if (installResult.exitCode !== undefined) {
            ...
            if (installResult.output)
                await logger.error('Install stdout available');
            if (installResult.error)
                await logger.error('Install stderr available');
        }
        else {
 ...
 */
export async function getDevCommandArgs(_sandbox, packageManager) {
